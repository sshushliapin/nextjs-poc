...
dotnet sitecore login --cm https://cm.poc.localhost/ --auth https://id.poc.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
# JSS sample has not been deployed yet. Use its deployment process to initialize.
    ...
    try {
        jss deploy items -c -d --acceptCertificate 71:94:9C:F2:23:07:35:A9:0B:B8:7B:20:B7:2F:13:59:64:F9:5A:54
    } finally {
        Pop-Location
    }
    if ($LASTEXITCODE -ne 0) {
...
Start-Process https://cm.poc.localhost/sitecore/
Start-Process https://www.poc.localhost/
